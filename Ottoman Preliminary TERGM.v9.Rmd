...
#Networks by ethnic groups
alb.coop.by.year.2 <- split(alb_net, alb_net$onset)
kurd.coop.by.year.2 <- split(kurd_net, kurd_net$onset)
bul.coop.by.year.2 <- split(bul_net, bul_net$onset)
...
alb_elites.2$ide.pos <- factor(alb_elites.2$ide.pos, levels = c(2,1,0))
...
# Plot again
ggplot(alb_elites.3, aes(x=year, y=n, fill=ide.pos)) + 
  geom_area() +
